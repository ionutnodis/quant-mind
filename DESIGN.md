...
| 2026-07-26 | Amber action buttons sanctioned: the ONE button per page whose result is book data (Hedge Run, WhatIf Compute, Lab Apply / Use in Apply) may use the amber-tinted button style; all other buttons stay neutral. Decorative amber (e.g. hover accents on non-book elements) remains forbidden | Batch-2 final review adjudication, user-approved; codifies the de-facto convention |
| 2026-07-26 | Lab Apply gated to rate-series fits (US10Y/US2Y/US3M): equity fits disable Apply with an honest note | Apply's Δbp exposure mapping is dimensionally wrong for price series; user-approved |
| 2026-07-26 | Radix adoption is lazy: hand-rolled popovers/sheets stay until a wave touches the component, then it migrates to the Radix primitive; no big-bang migration | Personal tool, single user; churn without capability not justified |
