...
export function CommandPalette() {
  ...
  return (
    ...
    >
      <div onClick={(e) => e.stopPropagation()} className="w-[440px]">
        <Command
        ...
        >
          <Command.Input
            autoFocus
            placeholder="Go to…"
            className="w-full bg-transparent px-4 py-3 text-ink outline-none border-b border-hairline placeholder:text-muted"
          />
          <Command.List className="max-h-72 overflow-auto py-1">
            ...
            {PAGES.map((p) => (
              <Command.Item
                ...
                className="px-4 py-2 text-[13px] text-ink data-[selected=true]:bg-surface data-[selected=true]:text-you cursor-pointer"
              >
                {p.label}
              </Command.Item>
            ))}
