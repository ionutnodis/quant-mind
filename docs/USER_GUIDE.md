# QuantMind user guide

[← Back to the README](../README.md)

The detailed reference for setting up a book, understanding each screen, and checking the evidence behind a result.

**Jump to:** [First session](#your-first-useful-session) · [Screens](#screen-by-screen-guide) · [World](#build-your-personal-world-desk) · [Practical workflows](#three-practical-workflows) · [Installation](#quick-start) · [Configuration](#configuration) · [Limits](#product-boundaries)
...
The **World monitor** adds a personal news-and-events desk: open macro, central-bank, energy, geopolitical and disaster feeds, filtered locally through your pinned book, watchlist and interests. Its public routes work without an IBKR connection or market-news subscription; optional social connectors have separate access requirements.
...
The detailed installation and broker acceptance checklist lives in the [first-user runbook](FIRST_USER_RUNBOOK.md).
...
![Current Setup screen showing the single next action and readiness cards for the API, broker, market cache, macro, options, FX, European ETF profiles, and current book](screenshots/setup-desktop.png)
...
<p align="center">
  <a href="screenshots/world-mobile.png"><img src="screenshots/world-mobile.png" alt="World monitor on a phone, with events and match explanations stacked for reading" width="260"></a>
</p>
...
X and Reddit are separate, disabled-by-default connectors. X requires explicit acceptance of its paid API plus a bearer token and query. Reddit requires approved OAuth access. Configure either only in server-side `.env`, never in frontend `VITE_` variables. See the [source catalog and setup guide](data-sources.md) for every endpoint, access requirement, supported coverage and limitations.

### What to read on Today

The top banner is operational: it tells you when the evidence cache is stale and provides the sync action. “Your book” vitals are placeholders in this alpha; use Portfolio for actual book values. “At a glance” is market context, not portfolio attribution. “Benchmark tail risk” describes the benchmark, not your portfolio.
...
![Today screen showing the evidence-age banner, market regime, book state, market context, overnight ranking, and benchmark tail-risk anchor](screenshots/today-desktop.png)
...
<details>
<summary>See the full-height Today phone view</summary>

<p align="center">
  <a href="screenshots/today-mobile.png"><img src="screenshots/today-mobile.png" alt="Full-height Today screen rendered as the read-only phone companion" width="240"></a>
</p>

Click the image for full-resolution detail. Its original proportions are preserved.
...
Jensen alpha is shown only when the cached risk-free series matches the reporting currency. The current alpha includes USD `US3M` evidence; an EUR, GBP, or other non-USD analysis therefore withholds alpha instead of silently subtracting a USD cash rate.

![Risk screen showing the factor builder, regression evidence, factor betas, variance decomposition, attribution, rolling beta, tail risk, and Monte Carlo controls](screenshots/risk-desktop.png)
...
- Foreign-position unrealized P&L remains local-currency-only in this alpha because current FX is not evidence of the acquisition-date cost rate.
...
5. For an equity-only book, test a resize in What-If or a beta target in Hedge Lab. If the book includes options, these tools refuse the calculation; inspect the option exposure in Portfolio instead.
...
The analytical core is deliberately separated from I/O. `risk/`, `analytics/`, and `hedge/` contain pure calculation code; broker, source, datastore, and API layers carry integration concerns. World uses a separate SQLite event cache in the local data directory; refreshing news does not rewrite portfolio or price evidence.

## Quick start

For the first live portfolio, follow the complete [first-user runbook](FIRST_USER_RUNBOOK.md). The short path is below.
...
### Try the illustrated World demo

This separate demo adds the illustrative events shown in the screenshots. Stop the earlier demo servers first, then run these commands from the repository root in two terminals.

Terminal 1:

```bash
uv run python scripts/world_demo.py
```
...
```bash
cd web
QM_API_PROXY_TARGET=http://127.0.0.1:8766 bun run dev -- --port 4173
```

Open `http://127.0.0.1:4173/world`. Events are clearly marked illustrative, the dataset is temporary, and feed requests are mocked. Use your normal local server for actual public-source ingestion.
...
The API keeps broker-reported account fields in their original currency and adds explicit `*_base` fields for normalized account totals. Base-currency analytical responses share one nested `fx` evidence contract (`status`, `base_currency`, `source`, `as_of`, `fetched_at`, and missing currencies).
...
Future design mockups live in [Product direction](PRODUCT_DIRECTION.md). They are not screenshots of implemented functionality. Keep them separate from the current-screen examples in this guide.

## Documentation

- [Design system and product decisions](../DESIGN.md)
- [First-user installation and acceptance runbook](FIRST_USER_RUNBOOK.md)
...
- [World source catalog, credentials and operating guide](data-sources.md)
- [World release engineering brief](WORLD_ENGINEERING_BRIEF.md)
