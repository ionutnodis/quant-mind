...
- **Alpha unavailable in a non-USD book:** this is intentional in v0.6. `US3M` is a USD risk-free series, so QuantMind withholds Jensen alpha when `QM_BASE_CURRENCY` is not USD rather than mixing cash-rate regimes.
